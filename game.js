...
function arm(parent,side,mat,tool){
 ...
 const fore=new THREE.Group();fore.position.set(0,-.98,0);fore.rotation.x=-.92;shoulder.add(fore);box(fore,[.58,1.05,.62],[0,-.42,0],mat);
 tool.position.set(0,-.78,-.05);fore.add(tool);return{shoulder,fore,tool,side};
}
function coreModel(){
 ...
 box(body,[.85,.7,.8],[0,4.35,0],M.green);box(body,[.12,.11,.06],[.2,4.4,-.43],M.pink);
 const la=arm(body,-1,M.yellow,sword(2.9)),ra=arm(body,1,M.black,sword(2.9));box(la.shoulder,[.7,.45,.72],[0,.2,0],M.green);
...
function sentinelModel(){
 ...
 const la=arm(body,-1,M.tan,gun()),ra=arm(body,1,M.tan,sword(3.25));
...
function restart(){clearBattle();health=100;score=wave=combo=0;over=false;spawnTimer=0;hero.position.set(0,0,8);$('#healthBar').style.width='100%';$('#healthText').textContent='100';$('#score').textContent='00';$('#gameOver').classList.remove('show');if(started)spawnWave()}
...
const clock=new THREE.Clock(),move=new THREE.Vector3(),target=new THREE.Vector3(),forward=new THREE.Vector3();
