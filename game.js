...
function sword(length=3){const r=new THREE.Group();r.userData.bladeLength=length;box(r,[.11,.1,length],[0,0,-length/2-.2],M.steel);const t=new THREE.Mesh(new THREE.ConeGeometry(.1,.42,4),M.steel);t.rotation.x=-Math.PI/2;t.position.z=-length-.4;r.add(t);box(r,[.5,.12,.12],[0,0,-.1],M.black);box(r,[.16,.16,.48],[0,0,.18],M.dark);return r}
...
function arm(parent,side,mat,tool){
 ...
 const fore=new THREE.Group();fore.position.set(0,-.98,0);fore.rotation.x=.92;fore.rotation.y=0;shoulder.add(fore);box(fore,[.58,1.05,.62],[0,-.42,0],mat);
...
const enemies=[],particles=[],shots=[],trails=[],keys=new Set();let health=100,score=0,wave=0,attacking=false,attackTime=0,attackId=0,attackSide=1,fired=false,combo=0,comboTimer=0,over=false,spawnTimer=0;
...
function slash(){if(started&&!menuOpen&&!over&&!attacking){attacking=true;attackTime=0;attackId++;attackSide=attackId%2?1:-1;fired=false;hero.userData.bladeSamples=null}}
function bladePoints(tool){const length=tool.userData.bladeLength;tool.updateWorldMatrix(true,false);return{base:tool.localToWorld(new THREE.Vector3(0,0,-.45)),tip:tool.localToWorld(new THREE.Vector3(0,0,-length-1.15))}}
function swordTrail(previous,current){const geometry=new THREE.BufferGeometry();const a=previous.base,b=previous.tip,c=current.base,d=current.tip;geometry.setAttribute('position',new THREE.Float32BufferAttribute([...a.toArray(),...b.toArray(),...d.toArray(),...a.toArray(),...d.toArray(),...c.toArray()],3));geometry.setAttribute('color',new THREE.Float32BufferAttribute([.25,.25,.25,1,1,1,1,1,1,.25,.25,.25,1,1,1,.25,.25,.25],3));geometry.computeVertexNormals();const material=new THREE.MeshBasicMaterial({color:selectedUnit==='core'?0xff5aa1:0xbcecff,vertexColors:true,transparent:true,opacity:.8,side:THREE.DoubleSide,blending:THREE.AdditiveBlending,depthWrite:false});const mesh=new THREE.Mesh(geometry,material);scene.add(mesh);trails.push({mesh,life:.23,max:.23});return[new THREE.Triangle(a,b,d),new THREE.Triangle(a,d,c)]}
function swordHits(triangles){const target=new THREE.Vector3(),closest=new THREE.Vector3();for(const e of enemies){if(e.userData.dead||e.userData.lastHit===attackId)continue;target.copy(e.position).add(new THREE.Vector3(0,1.65,0));if(triangles.some(t=>t.closestPointToPoint(target,closest).distanceTo(target)<1.1)){e.userData.lastHit=attackId;hit(e)}}}
function updateSwordTrail(){hero.updateWorldMatrix(true,true);const arms=selectedUnit==='core'?[hero.userData.la,hero.userData.ra]:[hero.userData.la],samples=arms.map(a=>bladePoints(a.tool)),previous=hero.userData.bladeSamples;if(previous){for(let i=0;i<samples.length;i++)swordHits(swordTrail(previous[i],samples[i]))}hero.userData.bladeSamples=samples}
...
function clearBattle(){enemies.forEach(e=>scene.remove(e));particles.forEach(p=>scene.remove(p.m));shots.forEach(s=>scene.remove(s.m));trails.forEach(t=>{scene.remove(t.mesh);t.mesh.geometry.dispose();t.mesh.material.dispose()});enemies.length=particles.length=shots.length=trails.length=0}
...
const clock=new THREE.Clock(),move=new THREE.Vector3(),target=new THREE.Vector3();let cameraAngle=Math.atan2(10,15);
function animateHero(dt,time,moving){
 ...
 if(attacking){
  ...
  if(p>=1){attacking=false;hero.userData.bladeSamples=null}
 }
 ...
 if(attacking&&attackTime/.72>=.22&&attackTime/.72<=.64)updateSwordTrail();else r.bladeSamples=null;
}
function update(dt,time){if(!hero)return;const canPlay=started&&!menuOpen&&!over;move.set(0,0,0);if(canPlay){cameraAngle+=((keys.has('q')?1:0)-(keys.has('e')?1:0))*1.65*dt;const ix=(keys.has('d')?1:0)-(keys.has('a')?1:0),iz=(keys.has('s')?1:0)-(keys.has('w')?1:0);move.set(ix*Math.cos(cameraAngle)+iz*Math.sin(cameraAngle),0,-ix*Math.sin(cameraAngle)+iz*Math.cos(cameraAngle))}const moving=move.lengthSq()>0;if(moving){move.normalize();hero.position.addScaledVector(move,(keys.has('shift')?9.5:6.2)*dt);hero.position.x=THREE.MathUtils.clamp(hero.position.x,-20.8,20.8);hero.position.z=THREE.MathUtils.clamp(hero.position.z,-20.8,20.8);const rr=Math.atan2(-move.x,-move.z);hero.rotation.y+=Math.atan2(Math.sin(rr-hero.rotation.y),Math.cos(rr-hero.rotation.y))*Math.min(1,dt*12)}animateHero(dt,time,moving);
 ...
 for(let i=trails.length-1;i>=0;i--){const t=trails[i];t.life-=dt;t.mesh.material.opacity=.8*Math.max(0,t.life/t.max);if(t.life<=0){scene.remove(t.mesh);t.mesh.geometry.dispose();t.mesh.material.dispose();trails.splice(i,1)}}
